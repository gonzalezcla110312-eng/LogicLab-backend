...
CREATE TABLE IF NOT EXISTS TipoPQRSF (
	...
	TipoPQRSF VARCHAR(50) NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS RegistroPQRSF (
	...
	created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT fk_pqrsf_tipo FOREIGN KEY (id_TipoPQRSF) REFERENCES TipoPQRSF(id_TipoPQRSF),
	CONSTRAINT fk_pqrsf_usuario FOREIGN KEY (atendido_por) REFERENCES usuarios(id)
);

INSERT INTO TipoPQRSF (TipoPQRSF, DescripcionTipo) SELECT 'PETICION','Solicitud formal de informacion o accion' WHERE NOT EXISTS (SELECT 1 FROM TipoPQRSF WHERE TipoPQRSF = 'PETICION');
INSERT INTO TipoPQRSF (TipoPQRSF, DescripcionTipo) SELECT 'QUEJA','Expresion de insatisfaccion con el servicio' WHERE NOT EXISTS (SELECT 1 FROM TipoPQRSF WHERE TipoPQRSF = 'QUEJA');
INSERT INTO TipoPQRSF (TipoPQRSF, DescripcionTipo) SELECT 'RECLAMO','Exigencia de correccion por inconformidad' WHERE NOT EXISTS (SELECT 1 FROM TipoPQRSF WHERE TipoPQRSF = 'RECLAMO');
INSERT INTO TipoPQRSF (TipoPQRSF, DescripcionTipo) SELECT 'SUGERENCIA','Propuesta de mejora al servicio' WHERE NOT EXISTS (SELECT 1 FROM TipoPQRSF WHERE TipoPQRSF = 'SUGERENCIA');
INSERT INTO TipoPQRSF (TipoPQRSF, DescripcionTipo) SELECT 'FELICITACION','Reconocimiento positivo al servicio' WHERE NOT EXISTS (SELECT 1 FROM TipoPQRSF WHERE TipoPQRSF = 'FELICITACION');
